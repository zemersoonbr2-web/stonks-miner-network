-- Update handle_new_user function to use referralNickname instead of referralCode
...
AS $function$
DECLARE
  referral_nickname_input TEXT;
  referrer_user_id UUID;
  user_language TEXT;
BEGIN
  -- Get referral nickname and language from metadata
  referral_nickname_input := NEW.raw_user_meta_data->>'referralNickname';
  ...
  -- Process referral if nickname was provided
  IF referral_nickname_input IS NOT NULL AND referral_nickname_input != '' THEN
    -- Find referrer by nickname
    SELECT id INTO referrer_user_id
    FROM public.profiles
    WHERE nickname = referral_nickname_input;
